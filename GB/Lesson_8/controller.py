    ...
    while True:
        ...
        if user_input == "з":
            if len(data_phonebook) != 0:  # проверка длины
                if user_request("вывод словаря").capitalize() == "Д":  # запрос на отображение словаря
                    show_list_dict(data_phonebook)  # показать словарь
                surname_filter = filter_records_surname(data_phonebook)  # включаем поиск по фамилии
                change_records(data_phonebook, surname_filter)  # функция замены по фамилии
            else:
                view_notion("пустой справочник")
